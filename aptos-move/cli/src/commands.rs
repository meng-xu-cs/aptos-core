...
use crate::{
    ...
    move_types::{
        ArgWithTypeJSON, ChunkedPublishOption, EntryFunctionArguments, EntryFunctionArgumentsJSON,
        LargePackagesModuleOption, MovePackageOptions, OverrideSizeCheckOption,
...
const HELLO_BLOCKCHAIN_EXAMPLE: &str =
    include_str!("../../move-examples/hello_blockchain/sources/hello_blockchain.move");
...
pub enum MoveTool {
    ...
    #[clap(subcommand)]
    Coverage(crate::coverage::CoveragePackage),
...
pub struct InitPackage {
    ...
    pub named_addresses: BTreeMap<String, MoveManifestAccountWrapper>,
...
impl CliCommand<&'static str> for TestPackage {
    ...
    async fn execute(self) -> CliTypedResult<&'static str> {
        ...
        let result = move_unit_test::package_test::run_move_unit_tests(
            ...
            crate::aptos_debug_natives::aptos_debug_natives(
...
async fn submit_chunked_publish_transactions(
    ...
    for (idx, payload) in payloads.into_iter().enumerate() {
        ...
        let result = dispatch_transaction(payload, txn_options, env).await;
...
impl CliCommand<TransactionSummary> for CreateResourceAccountAndPublishPackage {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        ...
        dispatch_transaction(payload, &txn_options, &env).await
    }
}
...
impl ArgWithType {
    ...
    /// Get the vector depth
    pub fn vector_depth(&self) -> u8 {
        self._vector_depth
    }
...
// ── WithMoveEnv implementations ──

macro_rules! impl_with_move_env {
    ($($ty:ident),+ $(,)?) => {
        $(
            impl crate::WithMoveEnv for $ty {
                fn attach_env(mut self, env: std::sync::Arc<MoveEnv>) -> Self {
                    self.env = env;
                    self
                }
            }
