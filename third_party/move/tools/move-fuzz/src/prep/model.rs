...
use crate::{
    ...
    prep::{
        ...
        datatype::{DatatypeRegistry, MapVariant, TypeBase, TypeItem, VectorVariant},
...
/// A node in the datatype provider graph (DPN)
enum DPGNode {
    Function(FunctionInst),
    Datatype(DatatypeItem),
}

/// An edge in the datatype provider graph (DPN)
enum DPGEdge {
    Use(usize),
    Def(usize),
    OptionToElement,
...
impl Model {
    ...
    pub fn provision(&mut self, pkgs: &[PkgDefinition], type_recursion_depth: usize) {
        ...
        for pkg in pkgs {
            ...
            let pkg_details = &pkg.as_built_package().package;
        ...
        // collect and register instantiations of entrypoint functions
        for decl in function_registry.iter_decls() {
            // only analyze primary functions
            if !decl.is_primary {
                continue;
            }
            ...
            if decl.generics.is_empty() {
                debug!("analyzing {}", decl.ident);
            } else {
                debug!("analyzing {}<{}>", decl.ident, decl.generics.len());
            }
            let insts =
            ...
            for inst in insts {
                debug!("analyzing instantiation {inst}");
        ...
        // populate the canvases for all primary functions
        for inst in self.function_inst_to_node_id.keys() {
            if generator.has_function_inst(inst) {
                continue;
            }

            generator.analyze_function_inst(inst);
        }
    }
    ...
    fn analyze_datatype_item(&mut self, item: DatatypeItem) -> NodeIndex {
        ...
        debug!("analyzing datatype item {}", TypeItem::from(item.clone()));
