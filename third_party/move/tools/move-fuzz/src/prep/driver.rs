...
use crate::prep::{
    ...
    datatype::{DatatypeRegistry, TypeBase, TypeItem, TypeUnifier, VectorVariant},
    function::{FunctionInst, FunctionRegistry},
};
...
impl<'a> DriverGenerator<'a> {
    ...
    pub fn analyze_function_inst(&mut self, inst: &FunctionInst) -> bool {
        let decl = self.function_registry.lookup_decl(&inst.ident);
        ...
        for (i, item) in ret_ty.iter().enumerate() {
            match item {
                ...
                TypeClosureItem::Base(TypeClosureBase::Complex(t)) => {
                    ...
                    // TODO
                    // need follow-up instructions as one of the return value need to be deposited
                    return false;
                },
