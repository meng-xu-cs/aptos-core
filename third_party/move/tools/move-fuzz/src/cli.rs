...
use crate::{
    deps::{self, PkgDeclaration, PkgDefinition, PkgManifest, Project},
    fuzzer,
...
pub fn run_on(
    ...
    match command {
        ...
        FuzzCommand::Auto {
            ...
            cmd_auto(&workdir, project, pkg_filter, seed, type_recursion_depth)?;
        },
    }
...
fn cmd_build(project: Project, pkg_filter: FilterPackage, dev_mode: bool) -> Result<()> {
    ...
    for pkg in pkg_filter.apply(pkgs)? {
        package::build(pkg.as_manifest(), &named_accounts, language, dev_mode)?;
    }
