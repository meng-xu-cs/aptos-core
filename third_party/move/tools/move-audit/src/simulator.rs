...
impl Simulator {
    ...
    pub fn add_module(&mut self, pkg_dir: PathBuf, module: CompiledModule) -> Result<()> {
        ...
        if self.modules.get(&module_addr).map_or(false, |l1| {
            l1.iter().any(|(_, l2)| l2.contains_key(&module_name))
        }) {
            bail!(
...
pub fn move_format(pkg_dir: &Path, config: Option<&Path>) -> Result<bool> {
    let mut command = Command::new(APTOS_BIN.as_path());
    command.args(["move", "fmt"]);
